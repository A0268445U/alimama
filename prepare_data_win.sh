...
docker run --rm -v /`pwd`/testbench://work -w //work --network none ${IMAGE}  make
#docker run --rm -v /`pwd`/testbench://work -w //work --network none ${IMAGE} ./gen_data localdata.json
docker run --rm -v /`pwd`/testbench://work -w //work --network alimama ${IMAGE} ./export_to_hdfs localdata.json
